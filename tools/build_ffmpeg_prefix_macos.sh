...
# --- libplacebo (built from upstream + local patches) -------------------------
# We build libplacebo from source so we can apply the
# tools/patches/libplacebo/*.patch series (notably the COLOR_BIT fix for
# MoltenVK MTLTexture import on the videotoolbox zero-copy path). The
...
LIBPLACEBO_PATCH_ROOT="$LOCAL_PATCH_ROOT/libplacebo"

if [[ "${LIBPLACEBO_SKIP_BUILD:-0}" != "1" ]]; then
  log "Fetching libplacebo $LIBPLACEBO_GIT_REF"
  require_cmd meson ninja
  rm -rf "$LIBPLACEBO_INSTALL_ROOT" "$LIBPLACEBO_BUILD_DIR"
  fetch_git_ref "$LIBPLACEBO_GIT_URL" "$LIBPLACEBO_GIT_REF" "$LIBPLACEBO_SOURCE_DIR"
  # libplacebo bundles fast_float / glad / Vulkan-Headers as git submodules;
  # fetch them so meson configure does not fail with "missing subproject".
  git -C "$LIBPLACEBO_SOURCE_DIR" submodule update --init --recursive --depth 1

  if [[ "${LIBPLACEBO_SKIP_PATCHES:-0}" != "1" && -d "$LIBPLACEBO_PATCH_ROOT" ]]; then
    while IFS= read -r patch_path; do
      log "Applying libplacebo patch $(basename "$patch_path")"
      apply_patch_with_fallback "$LIBPLACEBO_SOURCE_DIR" "$patch_path"
    done < <(find "$LIBPLACEBO_PATCH_ROOT" -maxdepth 1 -type f -name '*.patch' | sort)
  fi

  # Vulkan deps (headers + loader + MoltenVK) typically come from Homebrew on
  # macOS. Probe brew when the env vars are not set so meson can resolve
  # `vulkan` via pkg-config.
  libplacebo_pkg_paths=()
  for prefix_var in VULKAN_HEADERS_PREFIX VULKAN_LOADER_PREFIX MOLTENVK_PREFIX; do
    prefix_val="${!prefix_var:-}"
    if [[ -z "$prefix_val" ]] && command -v brew >/dev/null 2>&1; then
      case "$prefix_var" in
        VULKAN_HEADERS_PREFIX) prefix_val="$(brew --prefix vulkan-headers 2>/dev/null || true)" ;;
        VULKAN_LOADER_PREFIX)  prefix_val="$(brew --prefix vulkan-loader 2>/dev/null || true)" ;;
        MOLTENVK_PREFIX)       prefix_val="$(brew --prefix molten-vk 2>/dev/null || true)" ;;
      esac
    fi
    if [[ -n "$prefix_val" && -d "$prefix_val/lib/pkgconfig" ]]; then
      libplacebo_pkg_paths+=("$prefix_val/lib/pkgconfig")
    fi
  done

  libplacebo_meson_args=(
    setup "$LIBPLACEBO_BUILD_DIR" "$LIBPLACEBO_SOURCE_DIR"
    --prefix "$LIBPLACEBO_INSTALL_ROOT"
    --buildtype release
  ...
  append_flags_from_env LIBPLACEBO_EXTRA_MESON_FLAGS libplacebo_meson_args

  log "Building libplacebo"
  (
    if (( ${#libplacebo_pkg_paths[@]} > 0 )); then
      export PKG_CONFIG_PATH="$(join_by : "${libplacebo_pkg_paths[@]}")${PKG_CONFIG_PATH:+:$PKG_CONFIG_PATH}"
    fi
    meson "${libplacebo_meson_args[@]}"
    meson compile -C "$LIBPLACEBO_BUILD_DIR"
    meson install -C "$LIBPLACEBO_BUILD_DIR"
  )

  if [[ ! -f "$LIBPLACEBO_INSTALL_ROOT/lib/pkgconfig/libplacebo.pc" ]]; then
    echo "libplacebo build did not produce libplacebo.pc under $LIBPLACEBO_INSTALL_ROOT" >&2
    exit 1
  fi
fi
...
if [[ -d "$LIBPLACEBO_INSTALL_ROOT/lib/pkgconfig" ]]; then
  PKG_CONFIG_PATH_ENTRIES+=("$LIBPLACEBO_INSTALL_ROOT/lib/pkgconfig")
  CPPFLAGS_ENTRIES+=("-I$LIBPLACEBO_INSTALL_ROOT/include")
  LDFLAGS_ENTRIES+=("-L$LIBPLACEBO_INSTALL_ROOT/lib")
fi
